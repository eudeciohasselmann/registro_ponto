...
async function fetchRecords() {
  ...
  try {
    ...
    alert("Nao foi possível carregar os registros do servidor.");
    return [];
  }
}
...
async function saveTimeEntry() {
  ...
  for (const date in registrosPorData) {
    ...
    try {
      ...
      console.error("Erro:", error);
      alert(`Nao foi possível salvar o registro para a data ${date}.`);
...
async function deleteRecord(date) {
  if (
    ...
    try {
      ...
      console.error("Erro:", error);
      alert("Nao foi possível remover o registro.");
    }
  }
}
